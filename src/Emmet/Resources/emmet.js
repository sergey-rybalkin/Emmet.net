...
function isWhiteSpace$3(code) {
    return (
        code === 32 /* space */ || code === 9 /* tab */ || code === 160
    ); /* non-breaking space */
}
/**
...
function element$2(scanner, options) {
    ...
    while (readable$1(scanner)) {
        ...
        if (!elem.repeat && !isEmpty(elem) && consume$2(scanner, isRepeater)) {
            ...
            (attr =
                shortAttribute(scanner, "id", options) ||
                shortAttribute(scanner, "class", options) ||
                attributeSet(scanner))
        ) {
            if (!elem.attributes) {
    ...
    if (consume$2(scanner, isAttributeSetStart)) {
        ...
        while (readable$1(scanner)) {
            if ((attr = attribute(scanner))) {
                ...
                throw error$1(
                    scanner,
                    `Unexpected "${peek$3(scanner).type}" token`,
                );
            }
        }
        return attributes;
...
function shortAttribute(scanner, type, options) {
    if (isOperator$1(peek$3(scanner), type)) {
        ...
        if (options.jsx && text(scanner)) {
            ...
            attr.value = slice(scanner, scanner.start + 1, scanner.pos - 1);
        } else {
...
function elementName(scanner, options) {
    ...
    if (options.jsx && consume$2(scanner, isCapitalizedLiteral)) {
        ...
        while (readable$1(scanner)) {
            const { pos } = scanner;
            if (
                !consume$2(scanner, isClassNameOperator) ||
                !consume$2(scanner, isCapitalizedLiteral)
            ) {
                scanner.pos = pos;
                break;
            }
...
function isQuote$1(token, isSingle) {
    return Boolean(
        token &&
        token.type === "Quote" &&
        (isSingle == null || token.single === isSingle),
    );
}
...
function isElementName$1(token) {
    return (
        token.type === "Literal" ||
        token.type === "RepeaterNumber" ||
        token.type === "RepeaterPlaceholder"
    );
}
...
function field$2(scanner, ctx) {
    ...
    if (
        (ctx.expression || ctx.attribute) &&
        scanner.eat(Chars$3.Dollar) &&
        scanner.eat(Chars$3.CurlyBracketOpen)
    ) {
        scanner.start = scanner.pos;
        let index;
        let name = "";
        if (scanner.eatWhile(isNumber$1)) {
            ...
            name = scanner.eat(Chars$3.Colon)
                ? consumePlaceholder$2(scanner)
                : "";
        ...
        children: convertGroup(abbr, {
            ...
            getText(pos) {
                ...
                if (Array.isArray(options.text)) {
                    ...
                    value =
                        (_a = options.text) !== null && _a !== void 0 ? _a : "";
                }
                return value;
            },
            getVariable(name) {
...
function convertStatement(node, state) {
    ...
    if (node.repeat) {
        ...
        repeat.count =
            repeat.implicit && Array.isArray(state.text)
...
function field$1(scanner) {
    ...
    if (scanner.eat(Chars$2.Dollar) && scanner.eat(Chars$2.CurlyBracketOpen)) {
        ...
        if (scanner.eatWhile(isNumber$1)) {
            // It’s a field
            index = Number(scanner.current());
            name = scanner.eat(Chars$2.Colon)
                ? consumePlaceholder$1(scanner)
                : "";
        } else if (isAlpha$1(scanner.peek())) {
            // It’s a variable
...
function stringValue(scanner) {
    ...
    if (isQuote$2(ch)) {
        ...
        return {
            type: "StringValue",
            value: scanner.substring(
                start + 1,
                scanner.pos - (finished ? 1 : 0),
            ),
...
function operatorType(ch) {
    return (
        (ch === Chars$2.Sibling && OperatorType.Sibling) ||
...
function isBracket$1(code) {
    return (
        code === Chars$2.RoundBracketOpen || code === Chars$2.RoundBracketClose
    );
}
function isLiteral$1(code) {
    return (
        isAlphaWord(code) || code === Chars$2.Percent || code === Chars$2.Slash
    );
}
/**
...
function shouldConsumeDashAfter(token) {
    return (
        ...
        (token.type === "NumberValue" && !token.unit)
    );
}
/**
...
function consumeValue(scanner, inArgument) {
    ...
    while (readable(scanner)) {
        ...
        if (isValue(token)) {
            ...
            isValueDelimiter(token) ||
            (inArgument && isWhiteSpace$1(token))
        ) {
            scanner.pos++;
        } else {
            break;
    ...
    if (consume$1(scanner, isOpenBracket$1)) {
        ...
        while (readable(scanner) && !consume$1(scanner, isCloseBracket$1)) {
            if ((value = consumeValue(scanner, true))) {
                ...
                !consume$1(scanner, isWhiteSpace$1) &&
                !consume$1(scanner, isArgumentDelimiter)
            ) {
                throw error(scanner, "Unexpected token");
            }
        }
...
function parse$2(abbr, options) {
    try {
        const tokens =
            typeof abbr === "string"
                ? tokenize(abbr, options && options.value)
                : abbr;
    ...
    for (const child of node.children) {
        ...
        if (resolved) {
            ...
            if (isNode(deepest.node)) {
                deepest.node.children = deepest.node.children.concat(
                    walkResolve(child, resolve),
                );
            }
        } else {
            children.push(child);
...
function pushField(stream, index, placeholder) {
    ...
    _push(
        stream,
        field(index, placeholder, stream.offset, stream.line, stream.column),
    );
}
/**
...
function isBooleanAttribute(attr, config) {
    return (
        attr.boolean ||
        config.options["output.booleanAttributes"].includes(
...
function isInline(node, config) {
    ...
    return node.name
        ? isInline(node.name, config)
        : Boolean(node.value && !node.attributes);
}
/**
...
function lorem(node, ancestors, config) {
    ...
    if (node.name && (m = node.name.match(reLorem))) {
        ...
        const maxWordCount = m[3]
            ? Math.max(minWordCount, Number(m[3].slice(1)))
            : minWordCount;
...
function paragraph(dict, wordCount, startWithCommon) {
    ...
    while (totalWords < wordCount) {
        words = sample(
            dict.words,
            Math.min(rand(2, 30), wordCount - totalWords),
        );
        totalWords += words.length;
        result.push(sentence(insertCommas(words)));
    }
...
function expandShortNotation(node, ancestors, config) {
    ...
    for (let cl of data.classNames) {
        ...
        if ((m = cl.match(reElement))) {
            prefix =
                getBlockName(path, m[1].length, config.context) +
                options["bem.element"] +
                m[2];
...
function getBEMDataFromContext(context) {
    if (!context._bem) {
        context._bem = parseBEM(
            (context.attributes && context.attributes.class) || "",
        );
    }
    return context._bem;
}
...
function label(node) {
    if (node.name === "label") {
        const input = find$1(
            node,
            (n) => n.name === "input" || n.name === "textarea",
        );
        if (input) {
...
function createCommentState(config) {
    ...
    return {
        ...
        before: options["comment.before"]
            ? template(options["comment.before"])
            : void 0,
        after: options["comment.after"]
            ? template(options["comment.after"])
            : void 0,
    };
}
/**
            ...
            if (!pushSnippet(node, state, next)) {
                if (node.value) {
                    const innerFormat =
                        ...
                        startsWithBlockTag(node.value, config);
                ...
                if (!node.value && !node.children.length) {
                    const innerFormat =
                        config.options["output.formatLeafNode"] ||
                        config.options["output.formatForce"].includes(
                            node.name,
                        );
                    innerFormat && pushNewline(state.out, ++out.level);
...
function getIndent(state) {
    ...
    if (
        ...
        isSnippet(parent) ||
        (parent.name &&
            config.options["output.formatSkip"].includes(parent.name))
    ) {
        return 0;
    }
    return 1;
    ...
    if (value.length && typeof value[0] === "string") {
        ...
        if (
            (matches === null || matches === void 0
                ? void 0
                : matches.length) &&
            !config.options["inlineElements"].includes(matches[1].toLowerCase())
        ) {
            return true;
        }
    }
...
function element(node, index, items, state, next) {
    ...
    if (node.name && (node.name !== "div" || !primary.length)) {
        pushString(
            out,
            (options.beforeName || "") + node.name + (options.afterName || ""),
        );
    }
    ...
    for (const attr of attrs) {
        if (attr.value) {
            if (attr.name === "class") {
                ...
                const tokens = attr.value.map((t) =>
                    typeof t === "string" ? t.replace(/\s+/g, ".") : t,
                );
    ...
    if (attrs.length) {
        ...
        for (let i = 0; i < attrs.length; i++) {
            ...
            if (isBooleanAttribute(attr, config) && !attr.value) {
                if (
                    !config.options["output.compactBoolean"] &&
...
function pug(abbr, config) {
    return indentFormat(abbr, config, {
        ...
        beforeTextLine: "| ",
        selfClose:
            config.options["output.selfClosingStyle"] === "xml" ? "/" : "",
    });
}
...
function parse$1(abbr, config) {
    ...
    config.text =
        oldTextValue !== null && oldTextValue !== void 0
            ? oldTextValue
            : config.text;
    return abbr;
}
...
function nest(snippets) {
    ...
    for (const cur of snippets.filter(isProperty)) {
        ...
        while (stack.length) {
            ...
            if (
                cur.property.startsWith(prev.property) &&
...
function css(abbr, config) {
    ...
    if (
        ((_a = config.context) === null || _a === void 0 ? void 0 : _a.name) ===
...
function getSingleNumeric(node) {
    if (node.value.length === 1) {
        ...
        if (
            cssVal.value.length === 1 &&
            cssVal.value[0].type === "NumberValue"
        ) {
...
function parse(abbr, config) {
    var _a;
    const snippets =
        ((_a = config.cache) === null || _a === void 0
            ? void 0
            : _a.stylesheetSnippets) || convertSnippets(config.snippets);
    const result = [];
    if (config.cache) {
    ...
    if (!resolveGradient(node, config)) {
        ...
        if (isValueScope(config)) {
            ...
            const snippet = snippets.find(
                (s) =>
                    s.type === CSSSnippetType.Property &&
                    s.property === propName,
            );
            ...
            if (snippet) {
                const resolved =
                    snippet.type === CSSSnippetType.Property
...
function resolveAsProperty(node, snippet, config) {
    ...
    if (node.value.length) {
        ...
        // choice
        node.value =
            snippet.value.length === 1 || defaultValue.some(hasField)
                ? defaultValue
    ...
    for (const cssVal of node.value) {
        ...
        for (const token of cssVal.value) {
            if (token.type === "Literal") {
                value.push(
                    resolveKeyword(token.value, config, snippet, minScore) ||
                        token,
                );
            } else if (token.type === "FunctionCall") {
                // For function calls, we should find matching function call
                // and merge arguments
                const match = resolveKeyword(
                    token.name,
                    config,
                    snippet,
                    minScore,
                ...
                if (match && match.type === "FunctionCall") {
                    value.push(
                        Object.assign(Object.assign({}, match), {
                            arguments: token.arguments.concat(
                                match.arguments.slice(token.arguments.length),
                            ),
                        }),
                    );
                } else {
...
function resolveKeyword(kw, config, snippet, minScore) {
    ...
    if (snippet) {
        if (
            (ref = findBestMatch(kw, Object.keys(snippet.keywords), minScore))
        ) {
            return snippet.keywords[ref];
        }
        for (const dep of snippet.dependencies) {
            if (
                (ref = findBestMatch(kw, Object.keys(dep.keywords), minScore))
            ) {
    ...
    if (
        (ref = findBestMatch(
            kw,
            config.options["stylesheet.keywords"],
            minScore,
        ))
    ) {
        return literal(ref);
...
function hasField(value) {
    for (const v of value.value) {
        if (
            v.type === "Field" ||
            (v.type === "FunctionCall" && v.arguments.some(hasField))
        ) {
            return true;
        }
    }
        ...
        switch (v.type) {
            case "ColorValue":
                value.push(
                    field(
                        state.index++,
                        color(v, config.options["stylesheet.shortHex"]),
                    ),
                );
                break;
            case "Literal":
            ...
            case "FunctionCall":
                ...
                for (let i = 0, il = v.arguments.length; i < il; i++) {
                    value = value.concat(
                        wrapWithField(v.arguments[i], config, state).value,
                    );
                    if (i !== il - 1) {
                        value.push(literal(", "));
                    }
...
function isValueScope(config) {
    if (config.context) {
        return (
            config.context.name === CSSAbbreviationScope.Value ||
            !config.context.name.startsWith("@@")
        );
    }
    return false;
}
...
var markupSnippets = {
    ...
    "tarea:cr|textarea:cols:rows":
        "textarea[name=${1} id=${1} cols=${2:30} rows=${3:10}]",
    ...
    return Object.assign(
        Object.assign(
            Object.assign(
                Object.assign(
                    Object.assign(
                        Object.assign({}, defaultConfig[key]),
...
function consumeQuoted(scanner) {
    ...
    if (isQuote(quote)) {
        while (!sol(scanner)) {
            if (
                previous(scanner) === quote &&
                peek(scanner) !== Chars$1.Escape
            ) {
                return true;
            }
        }
...
function consumeAttribute(scanner) {
    return (
        consumeAttributeWithQuotedValue(scanner) ||
        consumeAttributeWithUnquotedValue(scanner)
    );
}
function consumeAttributeWithQuotedValue(scanner) {
    const start = scanner.pos;
    if (
        consumeQuoted(scanner) &&
        consume(scanner, Chars.Equals) &&
        consumeIdent(scanner)
    ) {
        return true;
    }
    scanner.pos = start;
...
function isIdent(ch) {
    return (
        ch === Chars.Colon || ch === Chars.Dash || isAlpha(ch) || isNumber(ch)
    );
}
/**
...
function isUnquotedValue(ch) {
    return (
        !isNaN(ch) && ch !== Chars.Equals && !isWhiteSpace(ch) && !isQuote(ch)
    );
}
...
function extractAbbreviation(line, pos = line.length, options = {}) {
    ...
    while (!sol(scanner)) {
        ...
        if (isCloseBrace(ch, opt.type)) {
            ...
            stack.includes(Brackets.SquareR) ||
            stack.includes(Brackets.CurlyR)
        ) {
...
function getStartOffset(line, pos, prefix) {
    ...
    while (!sol(scanner)) {
        if (
            consumePair(scanner, Brackets.SquareR, Brackets.SquareL) ||
            consumePair(scanner, Brackets.CurlyR, Brackets.CurlyL)
        ) {
            continue;
        }
        result = scanner.pos;
...
function isOpenBrace(ch, syntax) {
    return (
        ch === Brackets.RoundL ||
        (syntax === "markup" &&
            (ch === Brackets.SquareL || ch === Brackets.CurlyL))
    );
}
function isCloseBrace(ch, syntax) {
    return (
        ch === Brackets.RoundR ||
        (syntax === "markup" &&
            (ch === Brackets.SquareR || ch === Brackets.CurlyR))
    );
}
...
function loadPreferences(config) {
    ...
    config.options = { ...vsConfig.options, ...(config.options || {}) };
    vsConfig = config;
}

/**
 * Replaces the Emmet abbreviation at the given caret position with its expanded output.
 *
 * The abbreviation is detected via {@link extractAbbreviation} using the current line and
 * caret offset, then expanded via {@link expandAbbreviation}. The produced expansion is
 * spliced back into `codeLine` by replacing the extracted abbreviation range.
 *
 * If no abbreviation can be extracted at `caretPos`, this function returns `false`.
 *
 * @param {string} codeLine
 * The full line of text that potentially contains an Emmet abbreviation.
 *
 * @param {number} caretPos
 * Zero-based caret offset within `codeLine` used to locate the abbreviation.
 *
 * @param {'markup'|'stylesheet'|'jsx'} contentType
 * The syntax type used for extraction and expansion.
 * Use `'markup'` for HTML/XML-like output and `'stylesheet'` for CSS-like output.
 *
 * @param {string} contentToWrap
 * Optional text to be wrapped by the abbreviation expansion (pass an empty string to disable).
 *
 * @returns {string|false}
 * The updated `codeLine` with the abbreviation replaced by expanded output, or `false` if
 * no abbreviation was found at the given caret position.
 *
 * @example
 * // Expand a markup abbreviation at end of the line
 * replaceAbbreviation('ul>li*2', 7, 'markup', '');
 *
 * @example
 * // Wrap existing content with an abbreviation (depends on abbreviation semantics)
 * replaceAbbreviation('div.wrapper', 11, 'markup', '<span>Text</span>');
 */
function replaceAbbreviation(codeLine, caretPos, contentType, contentToWrap) {
    var abbreviation = extractAbbreviation(codeLine, caretPos, {
        type: contentType,
    });
    if (!abbreviation) {
        return false;
    ...
    vsConfig.text = contentToWrap;
    vsConfig.type = contentType;

    var replacement = expandAbbreviation(abbreviation.abbreviation, vsConfig);
    var retVal =
        codeLine.slice(0, abbreviation.start) +
        replacement +
        codeLine.slice(abbreviation.end);

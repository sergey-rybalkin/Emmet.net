...
namespace Emmet.Engine
{
    ...
    public class EngineWrapper : IDisposable
    {
        ...
        /// </summary>
        /// <param name="preferencesFile">Full path to the emmet preferences file.</param>
        public EngineWrapper(string preferencesFile)
        {
        ...
        /// </exception>
        public bool RunCommand(int cmdId, ICodeEditor view, bool formatCode = true)
        {
            if (!_initialized)
                InitializeEngine();
            ...
            if (cmdId is PackageIds.CmdIDExpandAbbreviation)
                ...
                view.ReplaceSelection(replacement, formatCode);
        ...
        {
            // Visual Studio view types: cshtml = Razor
            // html = HTMLProjection
            // css = css
            // scss = scss
            // jsx = javascriptreact
            ...
            if (contentType.EndsWith("ss"))
                return "stylesheet";
            else if (contentType.EndsWith("react"))
                return "jsx";
            else
                return "markup";
        }
        ...
        private void InitializeEngine()
        {
            ...
            EngineCompiler compiler = new EngineCompiler();

            compiler.CompileCore(_engine);
            if (null != _preferencesFile)
                compiler.LoadExtensions(_preferencesFile, _engine);
